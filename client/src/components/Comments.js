...
export default function Comments() {
  ...
  async function handleAddComment(content) {
    try {
      ...
      const updatedComments = [data.comment, ...comments];  // 방금 작성한 댓글이 위에 위치하게  comments보다 앞에 작성
  ...
  return (
    <div className="px-3">
      ...
      {!isLoaded && <Spinner />}  {/* 댓글을 가져오는 중일 때 스피너 모양 출력 */}
...
function Form({ handleAddComment }) {
  ...
  async function handleSubmit(e) {  // 폼 제출
    try {
      e.preventDefault();
  ...
  function handleChange(e) {  // 댓글 내용 관리
