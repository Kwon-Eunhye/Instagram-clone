	...
	return (
		<div className="mt-4 bg-white">
			<div className="px-2 mb-2 flex justify-between items-center">
				{/* 아바타 */}
				<Link to={`/profiles/${article.author.username}`} className="inline-flex items-center">
					<img src={`${process.env.REACT_APP_SERVER}/files/profiles/${article.author.image}`} className="w-10 h-10 object-cover rounded-full" />
					<span className="ml-2">{article.author.username}</span>
				</Link>

				{active && modal}

				<svg className="w-1 cursor-pointer" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 128 512" onClick={() => setActive(true)}>
			...
			<div className="mt-2 px-2">
				<div className="flex">
					{/* 하트 */}
					{article.isFavorite ? (
						<svg className="w-6 fill-red-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" onClick={() => handleUnfavorite(article.id)}>
							<path d="M0 190.9V185.1C0 115.2 50.52 55.58 119.4 44.1C164.1 36.51 211.4 51.37 244 84.02L256 96L267.1 84.02C300.6 51.37 347 36.51 392.6 44.1C461.5 55.58 512 115.2 512 185.1V190.9C512 232.4 494.8 272.1 464.4 300.4L283.7 469.1C276.2 476.1 266.3 480 256 480C245.7 480 235.8 476.1 228.3 469.1L47.59 300.4C17.23 272.1 .0003 232.4 .0003 190.9L0 190.9z" />
						</svg>
					) : (
						<svg className="w-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" onClick={() => handleFavorite(article.id)}>
				...
				{article.commentCount > 0 && (
					<p className="text-gray-400 text-sm my-2">
						<Link to={`/p/${article.id}/comments`}>View all {article.commentCount}</Link>
					</p>
				)}
