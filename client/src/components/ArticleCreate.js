...
export default function ArticleCreate({ active, setActive }) {
  ...
  const [files, setFiles ] = useState([]);  // 업로드할 사진
  ...
  // 모달 창 닫기 버튼  // 모달 컨테츠 내용 이외의 화면을 클릭했을 때 닫힘
  function close(e) {
    ...
    return (
      <div className='fixed inset-0 bg-black/[0.2] z-10' onClick={close}>
        ...
        >
          ...
          <div className='p-4'>
            <label className='inline-block mb-2 font-semibold test-sm px-4 py-2 bg-gray-200 rounded-lg cursor-pointer'>
              <input
                type='file'
                className='hidden'
                onChange={({ target }) => setFiles(Array.from(target.files))} // Array.from() : 객체의 타입을 Array로 변경
                multiple={true} // 파일을 여러개 선택하여 업로드 가능하게하는 input의 속성
