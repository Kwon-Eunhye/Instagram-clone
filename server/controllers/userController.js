...
exports.register = [
  ...
  async (req, res, next) => {
    try {
      ...
      if (userByUsername) {
        const err = new Error('Username already is use');
        err.status = 400;
        throw err;
      }

      // 이메일 중복검사
      const userByEmail = await User.findOne({ email });  //req.body에서 중복되는 내용이있으면

      if (userByEmail) {  
        const err= new Error('E-mail already is use');
        err.status = 400;
        throw err;
      }
      ...
      res.json({ user }); // 가운데 status가 생략된 것은 200이라고 간주
...
// 정보 수정
exports.edit = [
  fileHandler('profiles').single('image'),  //single : 프로필 사진은 한개만 처리하기 때문에 사용
  async (req, res, next) => {
    try {

      // req.user: 로그인한 유저
      const _user = await User.findById(req.user._id);  //Id로 유저 검색
      ...
      // 계정 업데이트
      Object.assign(_user, req.body); // _user 정보 중 req.body에 담겨있는 정보만 업데이트
